...
if ( ! class_exists( '\\SkyVerge\\WooCommerce\\PluginUpdater\\Updater' ) ) :
	...
	class Updater {
		 ...
		 */
		public function __construct( $_plugin_file, $_api_data, $_api_url = 'https://skyverge.com/' ) {
			global $skyverge_plugin_data;
			...
			$this->cache_key   = 'sv_sl_' . md5( serialize( $this->slug . $this->api_data['license'] . $this->beta ) );

			$skyverge_plugin_data[ $this->slug ] = $this->api_data;
		...
		public function show_update_notification( $file, $plugin ) {
			...
			if ( ! empty( $update_cache->response[ $this->name ] ) && version_compare( $this->version, $version_info->new_version, '<' ) ) {
				...
				if ( empty( $version_info->download_link ) ) {
					...
					printf(
						__( 'There is a new version of %1$s available. %2$sView version %3$s details%4$s.', 'skyverge-plugin-updater' ),
					...
					printf(
						__( 'There is a new version of %1$s available. %2$sView version %3$s details%4$s or %5$supdate now%6$s.', 'skyverge-plugin-updater' ),
						esc_html( $version_info->name ),
						'<a target="_blank" class="thickbox" href="' . esc_url( $changelog_link ) . '">',
						esc_html( $version_info->new_version ),
						'</a>',
		...
		public function plugins_api_filter( $_data, $_action = '', $_args = null ) {
			...
			$cache_key = 'sv_api_request_' . md5( serialize( $this->slug . $this->api_data['license'] . $this->beta ) );
		...
		public function show_changelog() {
			...
			if ( ! current_user_can( 'update_plugins' ) ) {
				wp_die( __( 'You do not have permission to install plugin updates', 'skyverge-plugin-updater' ), __( 'Error', 'skyverge-plugin-updater' ), array( 'response' => 403 ) );
			}

			$data         = $skyverge_plugin_data[ $_REQUEST['slug'] ];
			$beta         = ! empty( $data['beta'] ) ? true : false;
			$cache_key    = md5( 'skyverge_plugin_' . sanitize_key( $_REQUEST['plugin'] ) . '_' . $beta . '_version_info' );
